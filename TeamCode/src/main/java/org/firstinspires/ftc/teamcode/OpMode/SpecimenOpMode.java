...
public class SpecimenOpMode {
    public static double linearToExpo(double input) {
        return (input >= 0) ? input*input : -input*input;
    }
    public SpecimenOpMode(LinearOpMode opMode, Gamepad gamepad1, Gamepad gamepad2) {
        this.opMode = opMode;
        ...
        while (opMode.opModeIsActive()) {
            ...
            if (gamepad1.isDown(GamepadKeys.Button.B)) {
                ...
                if (gamepad2.getTrigger(GamepadKeys.Trigger.RIGHT_TRIGGER) >= 0.4) { //split
                    ...
                    claws.setArmTargetPosition(DifferentialClaws.ClawPositionState.MIN.state);
                    telemetry.addData("target pos", DifferentialClaws.ClawPositionState.MIN.state);
                    ...
                    telemetry.addData("target pos", DifferentialClaws.ClawPositionState.MAX.state);
                    ...
                    telemetry.addData("target pos", DifferentialClaws.ClawPositionState.SPIT_STATE.state);
                }
